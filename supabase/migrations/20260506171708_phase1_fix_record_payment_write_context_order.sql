-- Corrige la regresion introducida por el hardening de Fase 1:
-- record_payment() hace SELECT ... FOR UPDATE sobre loans/installments y esas
-- policies ahora requieren el contexto efimero de escritura antes de tomar locks.
-- Riesgo si se mueve otra vez: el RPC puede ver la fila por SELECT simple pero
-- fallar con loan_not_found o installment_not_found_for_loan bajo RLS endurecido.
...
'RPC V1 atomico e idempotente por device_local_id. Abre el contexto efimero de escritura antes de los FOR UPDATE para convivir con el hardening RLS sin reabrir DML directo por Data API.';
