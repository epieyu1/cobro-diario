-- BR-6 no debe romper el contrato visible de BR-5.
-- Esta correccion restaura los codigos de error ya consumidos por UI/compuertas
-- y vuelve a usar el write-context propio de reverse_payment para no acoplarlo
-- accidentalmente al contexto interno de record_payment.
...
begin
  ...
  -- Igual que en BR-5, el contexto debe abrirse antes del primer FOR UPDATE.
  -- PostgreSQL sigue siendo la fuente de verdad y RLS no debe volver invisible
  -- la fila del pago ni permitir UPDATE directo fuera del RPC.
...
'RPC BR-5/BR-6 online-only para admin activo. Revierte un pago confirmado restaurando outstanding_amount y cada saldo por componente en la misma transaccion sin romper el contrato visible de BR-5.';
